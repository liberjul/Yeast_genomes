...
if os.path.exists(F"{data_dir}/info/mbm.new.accession_metadata.json"):
    with open(F"{data_dir}/info/mbm.new.accession_metadata.json", "r") as ifile:
        metadata_dict.update(json.load(ifile))
if os.path.exists(F"{data_dir}/info/outgroup.new.accession_metadata.json"):
    with open(F"{data_dir}/info/outgroup.new.accession_metadata.json", "r") as ifile:
        metadata_dict.update(json.load(ifile))
...
with open(F"{data_dir}/info/class.updated.accession_metadata.json", "w") as ofile:
...
for reg in regions:
    with open(F"{data_dir}/aligned_seqs/class.yeast_seqs.{reg}.mafft.clipkit.fasta", "r") as ifile:
        seq_dict = {}
        min_length = 9999
        max_length = 0
        seq = ""
    ...
    for i in strain_list:
        if i in seq_dict_strain:
            ...
            buffer = F"{buffer}>{i}|{reg}\n{'-'*max_length}\n"
    with open(F"{data_dir}/aligned_seqs/class.yeast_seqs.{reg}.mafft.ordered.fasta", "w") as ofile:
        ofile.write(buffer)
