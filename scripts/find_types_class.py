...
def remove_others(seq_dict, acc_list):
    excluded_headers = []
    for header in seq_dict:
        excl_list = [x for x in acc_list if x in header]
        if "NR_" in header:
            excluded_headers.append(header)
        elif len(excl_list) > 0:
            excluded_headers.append(header)
    return excluded_headers
...
def main():
    ...
    #             types.add(j)
    #             types.add(j.replace(" ", ""))
    with open(F"{data_dir}/info/seqs_to_remove.txt", "r") as ifile: # Sequences determined to be incorrectly classified
        lines = ifile.readlines()
        accs_to_remove = [x.strip() for x in lines]
    ...
    for reg in regions:
        ...
        with open(F"{data_dir}/aligned_seqs/class.yeast_seqs.{reg}.mafft.manual.fas.nex", "r") as ifile:
            ...
            print("Accessions not in metadata dict: ", accs_not_in_dict)
            ...
            print([x for x in all_binoms if "Atractocolax" in x])
        ...
        excluded_headers.extend(remove_others(seq_dict, accs_to_remove))
        if exclusion_str != "":
