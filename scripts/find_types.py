...
with open(F"{data_dir}/info/updated.accession_metadata.json", "r") as ifile:
...
for reg in regions:
    buffer = ""
    with open(F"{data_dir}/aligned_seqs/yeast_seqs.{reg}.mafft.ordered.fasta", "r") as ifile:
        seq = ""
        line = ifile.readline()
        while line != "":
            if line != "" and line[0] == ">":
        ...
        if seq != "" and header_short in type_headers:
            buffer += F"{header}{seq}\n"
    with open(F"{data_dir}/aligned_seqs/yeast_seqs.{reg}.mafft.ordered.types.fasta", "w") as ofile:
        ofile.write(buffer)
